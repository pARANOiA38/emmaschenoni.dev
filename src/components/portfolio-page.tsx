...
export function PortfolioPage({ locale }: { locale: Locale }) {
  ...
  return <html lang={locale}><body><main>
    <header className="site-header"><a href={home} className="brand" aria-label="Emmanuel Schenoni home">E. Schenoni</a><nav className="desktop-nav" aria-label="Main navigation">{t.nav.map(([label, target]) => <a href={`#${target}`} key={target}>{label}</a>)}</nav><div className="header-actions"><div className="language" aria-label="Language"><a href={locale === "en" ? "/" : undefined} className={locale === "es" ? "active" : ""} aria-current={locale === "es" ? "page" : undefined}>ES</a><span>/</span><a href={locale === "es" ? "/en" : undefined} className={locale === "en" ? "active" : ""} aria-current={locale === "en" ? "page" : undefined}>EN</a></div><ThemeToggle labels={t.theme} /><MobileNav links={t.nav} /></div></header>
    ...
    <section id="experience" className="section"><SectionIntro label={t.experience.label} title={t.experience.title} /><p className="career-intro">{t.experience.intro}</p><div className="experience-list">{experience.map((job, index) => <article className={`job ${job.featured ? "featured" : ""}`} key={`${job.company}-${job.period}`}><span className="job-number">0{index + 1}</span><div><h3>{job.company}</h3><p>{job.roles[locale]}</p><p className="job-description">{t.experience.items[job.id]}</p></div><time>{job.period === "2025 — Present" ? `2025 — ${t.experience.current}` : job.period}</time></article>)}</div></section>
    <section id="expertise" className="section"><SectionIntro label={t.skills.label} title={t.skills.title} /><div className="skills-grid">{skills.map(group => <article className="skill-group" key={group.key}><h3>{skillNames[group.key]}</h3><ul>{group.items.map(skill => <li key={skill}>{skill}</li>)}</ul></article>)}</div><aside className="ai-support"><Sparkles size={17} aria-hidden="true" /><div><h3>{t.skills.aiTitle}</h3><p>{t.skills.aiBody}</p></div></aside></section>
